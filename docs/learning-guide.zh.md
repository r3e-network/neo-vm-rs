...
| 输入 | NeoVM 字节码、初始栈、系统调用宿主回调 |
| 职责 | 解码标准操作码、执行栈与状态语义、暴露可复用运行时 API |
| 输出 | halt/fault 结果、最终栈、gas/计费证据 |
| 使用者 | neo-riscv-vm、neo-zkvm、Neo N4 执行核心 |
...
| `src/semantics/runtime/comparison.rs` | 执行 runtime、状态转换或 gas 行为 | 20 | 0 |
| `src/semantics/collections.rs` | 实现细节或辅助模块 | 19 | 0 |
| `src/interpreter/runtime_types.rs` | 执行 runtime、状态转换或 gas 行为 | 16 | 0 |
| `src/semantics/runtime/stack.rs` | 执行 runtime、状态转换或 gas 行为 | 16 | 0 |
| `src/semantics/comparison.rs` | 实现细节或辅助模块 | 13 | 0 |
| `src/interpreter/api.rs` | VM 解释器和 opcode 语义 | 11 | 1 |
| `src/vm/opcode.rs` | opcode 元数据、定价或标准解码规则 | 7 | 0 |
| `src/abi/fast_codec.rs` | 线格式、栈值或 host/guest 边界类型 | 3 | 8 |
| `src/interpreter/helpers/retained.rs` | VM 解释器和 opcode 语义 | 4 | 4 |
| `src/interpreter/helpers/bridge.rs` | VM 解释器和 opcode 语义 | 5 | 0 |
| `src/interpreter/helpers/mod.rs` | VM 解释器和 opcode 语义 | 5 | 0 |
| `src/semantics/runtime/byte_ops.rs` | 执行 runtime、状态转换或 gas 行为 | 5 | 0 |
| `src/semantics/runtime/conversion.rs` | 执行 runtime、状态转换或 gas 行为 | 4 | 0 |
| `src/vm/limits.rs` | 实现细节或辅助模块 | 4 | 0 |
| `src/abi/callback_codec.rs` | 线格式、栈值或 host/guest 边界类型 | 3 | 0 |
| `src/abi/execution.rs` | 线格式、栈值或 host/guest 边界类型 | 3 | 0 |
| `src/host/syscall.rs` | 宿主 syscall 契约与分发边界 | 2 | 3 |
| `src/semantics/runtime/mod.rs` | 执行 runtime、状态转换或 gas 行为 | 3 | 0 |
| `src/abi/result_codec.rs` | 线格式、栈值或 host/guest 边界类型 | 2 | 0 |
| `src/semantics/conversion.rs` | 实现细节或辅助模块 | 2 | 0 |
| `tests/runtime_opcode_ops.rs` | 外部行为或集成测试 | 0 | 6 |
| `tests/abi_opcode_semantics.rs` | 外部行为或集成测试 | 0 | 5 |
| `tests/shared_semantics.rs` | 外部行为或集成测试 | 0 | 5 |
| `tests/source_layout.rs` | 外部行为或集成测试 | 0 | 5 |
| `tests/interpreter_smoke.rs` | 外部行为或集成测试 | 0 | 4 |
| `src/interpreter/executor.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/byte_ops.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/compound_ops.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/control.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/numeric_ops.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/push_ops.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/result_ops.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/slot_ops.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `src/interpreter/executor/stack_ops.rs` | VM 解释器和 opcode 语义 | 1 | 0 |
| `tests/boundary_codecs.rs` | 外部行为或集成测试 | 0 | 2 |
| `tests/canonical_opcode_vectors.rs` | 外部行为或集成测试 | 0 | 2 |
| `tests/syscall_vectors.rs` | 外部行为或集成测试 | 0 | 2 |
| `tests/wire_semantics.rs` | 外部行为或集成测试 | 0 | 2 |
| `tests/neo_vm_3_9_conformance.rs` | 外部行为或集成测试 | 0 | 1 |
| `tests/official_neo_vm_3_9_vmut.rs` | 外部行为或集成测试 | 0 | 1 |
| `benches/semantics.rs` | 实现细节或辅助模块 | 0 | 0 |
| `src/abi/mod.rs` | 线格式、栈值或 host/guest 边界类型 | 0 | 0 |
| `src/host/mod.rs` | host 侧编排与原生集成 | 0 | 0 |
| `src/interpreter/mod.rs` | VM 解释器和 opcode 语义 | 0 | 0 |
| `src/interpreter/opcodes.rs` | VM 解释器和 opcode 语义 | 0 | 0 |
| `src/semantics/mod.rs` | 实现细节或辅助模块 | 0 | 0 |
| `src/vm/mod.rs` | 实现细节或辅助模块 | 0 | 0 |
...
2. 读 `src/abi/stack_value.rs`：线格式、栈值或 host/guest 边界类型。
3. 读 `src/interpreter/state.rs`：VM 解释器和 opcode 语义。
4. 读 `src/interpreter/helpers/values.rs`：VM 解释器和 opcode 语义。
5. 读 `src/runtime/mod.rs`：执行 runtime、状态转换或 gas 行为。
6. 读 `src/semantics/arithmetic.rs`：实现细节或辅助模块。

## 修改安全清单

- 保持职责边界不变：解码标准操作码、执行栈与状态语义、暴露可复用运行时 API。
